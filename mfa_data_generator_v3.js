...
const CONFIG = {
    ...
    // ============================================================
    // Set your target compliance % - generator handles the rest!
    // ============================================================
    
    // YOUR MAIN KPI - Current Month Target Compliance
    CURRENT_MONTH_TARGET_COMPLIANCE: 0.95,    // 95% - What dashboard will show
    
    // Historical trend settings
    MIN_STARTING_COMPLIANCE: 0.60,             // 60% compliance 12 months ago
    COMPLIANCE_GROWTH_TYPE: 'logarithmic',     // 'linear', 'logarithmic', 'sigmoid'
    
    // Variation settings
    ALLOW_MONTHLY_VARIANCE: true,              // Natural month-to-month variation
    MONTHLY_VARIANCE_RANGE: 0.02,              // ±2% random variance
    ...
    SIGNIN_SOURCES: [
        { name: 'Okta', weight: 0.25 },
        { name: 'Azure AD', weight: 0.50 },
...
function getMFAComplianceTarget(date) {
    // Returns KPI-driven compliance target for a given date
    // Automatically generates realistic historical trend
    
    const currentDate = new Date('2024-10-21');
    const monthsAgo = Math.round((currentDate - date) / (30 * 24 * 60 * 60 * 1000));
    const totalMonths = 12;
    
    const clampedMonthsAgo = Math.max(0, Math.min(monthsAgo, totalMonths));
    const progress = 1 - (clampedMonthsAgo / totalMonths);  // 0 to 1
    
    const minCompliance = CONFIG.MIN_STARTING_COMPLIANCE;
    const maxCompliance = CONFIG.CURRENT_MONTH_TARGET_COMPLIANCE;
    const range = maxCompliance - minCompliance;
    
    let targetCompliance;
    
    // Apply growth curve
    if (CONFIG.COMPLIANCE_GROWTH_TYPE === 'linear') {
        targetCompliance = minCompliance + (range * progress);
    } else if (CONFIG.COMPLIANCE_GROWTH_TYPE === 'logarithmic') {
        targetCompliance = minCompliance + (range * Math.log(1 + progress * 9) / Math.log(10));
    } else if (CONFIG.COMPLIANCE_GROWTH_TYPE === 'sigmoid') {
        const sigmoid = 1 / (1 + Math.exp(-10 * (progress - 0.5)));
        targetCompliance = minCompliance + (range * sigmoid);
    } else {
        targetCompliance = minCompliance + (range * Math.log(1 + progress * 9) / Math.log(10));
    }
    
    // Add monthly variance if enabled
    if (CONFIG.ALLOW_MONTHLY_VARIANCE) {
        const monthIndex = date.getMonth();
        const variance = (((monthIndex * 37) % 100) / 100 - 0.5) * 2 * CONFIG.MONTHLY_VARIANCE_RANGE;
        targetCompliance += variance;
    }
    
    // Ensure within bounds
    return Math.max(0.5, Math.min(0.99, targetCompliance));
}
...
function generateMFARecord(recordNumber, currentDate) {
    ...
    // Determine if MFA was used - V3 KPI-driven with bug fixes
    let isMFA;
    const targetCompliance = getMFAComplianceTarget(signinDate);
    
    if (application.mfaRequired) {
        // App requires MFA - use target compliance, capped at 99%
        const mfaProbability = Math.min(targetCompliance, 0.99);
        isMFA = rng.next() < mfaProbability;
    } else if (employee.isAdmin) {
        // Admin users have high MFA usage
        const adminProbability = Math.min(Math.max(targetCompliance, CONFIG.ADMIN_MFA_ENFORCEMENT), 0.99);
        isMFA = rng.next() < adminProbability;
    } else {
        // Regular users on non-MFA apps
        const regularProbability = Math.min(targetCompliance * 0.5, 0.80);
        isMFA = rng.next() < regularProbability;
    }
...
const currentDate = new Date('2024-10-21'); // Current date for the simulation
let records = [];
...
function correctDataToTarget(records, currentDate) {
    ...
    const currentMonthRecords = records.filter(r => {
        const tlDate = new Date(r.TL_DATE);
        return tlDate.getMonth() === currentMonth && tlDate.getFullYear() === currentYear;
    });
    
    if (currentMonthRecords.length === 0) {
        console.error('   ⚠️  No current month records found');
        return records;
    }
    ...
    if (diff <= CONFIG.CORRECTION_TOLERANCE) {
        console.error(`   ✅ Already within tolerance`);
        return records;
    }
    ...
    console.error(`   Adjusting ${needToFlip} records...`);
...
const statsOutput = `
${'='.repeat(80)}
🔐 MFA DATA GENERATOR - STATISTICS SUMMARY (v3.3)
${'='.repeat(80)}
